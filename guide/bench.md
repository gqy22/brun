...
medium 扩展入口（默认 1/2/4/8/16 线程和 naive，各运行一次）：
...
2026-07-16 使用 Brun/cgroup 扩展到 8/16 线程：1/2/4/8/16 线程分别耗时
290.53/190.23/139.79/128.04/120.42 秒，线程翻倍的后两段收益降至 8.41% 和 5.95%；
`--naive` 为 8.38 秒。所有方案的解码记录 SHA-256 一致。实验开始时 load 1m 达 158.28，
因此当前只把 8 线程视为可能的效率点，不作为稳定推荐。详情见
`reports/bcftools-concat-threads-2026-07-16.md`。
